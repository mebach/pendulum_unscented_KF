...
class UnscentedKalmanFilter:
    def __init__(self):
        # Initial state and covariance estimate (random non zero values, but close to true
        # initial state)
        self.x = np.array([0.3,
                          0.2])

        self.n = len(self.x)  # Number of dimensions of the state vector
        ...
        # Initialize posterior estimates
        ...
        # Process noise covariance matrix (tune for expectation of imperfections in dynamic model)
        self.Q = np.eye(self.n) * 1e-5

        # Measurement noise covariance (tune for expectation of noise in measurements | only 1x1
        # because there is only one measurement
        self.R = np.array([[0.1]])
    ...
    def h(self, x):
        # Transorm from state space into measurement space
        r = np.sqrt(P.h**2 + P.l**2 - 2*P.h*P.l*np.cos(x[0]))
        return r

    #### Unscented Transform for prior ####
    ...
    #### Unscented Transform for measurments ####
